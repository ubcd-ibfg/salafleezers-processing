...
| Moving average | `windowFilter(@mean, data, [], N)` | `scipy.ndimage.uniform_filter1d(data, N)` | `utils/signal.py` |
| Block average | `windowFilter(@mean, data, hw, N)` | `data.reshape(n_blocks, N).mean(axis=1)` | `utils/signal.py` |
| Block sum | `windowFilter(@sum, data, [], N)` | `data.reshape(n_blocks, N).sum(axis=1)` | `utils/signal.py` |
...
### 6. Moving Average: `windowFilter(@mean, …)` → `scipy.ndimage.uniform_filter1d`

| | Notes |
|---|---|
| **Pro** | `uniform_filter1d` uses a compiled C kernel; far faster than Python loops. |
| **Pro** | Handles arbitrary array lengths without padding logic. |
| **Con** | MATLAB's `smooth(x, N)` uses a triangular kernel at array edges (shrinking window). `uniform_filter1d(mode='nearest')` uses edge-replication with a full-width box, giving slightly different values in the first and last `N//2` samples. |
| **Behavioral difference** | Interior values are identical. Edge values differ by <1% for typical window sizes used in this pipeline (N ≤ 5 for smoothing offsets). This difference is negligible in practice since offset data is dense. |
...
|---|---|---|
| `smooth` edge kernel (triangular vs. box) | <1% at array edges | `utils/signal.py` |
